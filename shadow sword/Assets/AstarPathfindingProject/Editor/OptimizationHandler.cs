...
namespace Pathfinding {
	...
	public class OptimizationHandler {
		...
		static string GetAstarPath () {
			#if UNITY_EDITOR
			if (IsJsEnabled ()) {
				return Application.dataPath + "/Plugins/AstarPathfindingProject";
			} else {
		...
		public static void FindDefines (Dictionary<string,DefineObject> defines) {
			...
			folders = new string[] { GetAstarPath () };

			for (int i=0;i<folders.Length;i++) {
				FindDefines (folders[i],defines);
			}

			List<string> toRemove = new List<string>();
			foreach (KeyValuePair<string, DefineObject> define in defines ) {
				if ( define.Value.brief.Contains ("@SHOWINEDITOR") ) {
					define.Value.brief =  define.Value.brief.Replace ("@SHOWINEDITOR", "").Trim ();
					if ( define.Value.description != null ) define.Value.description =  define.Value.description.Replace ("@SHOWINEDITOR", "").Trim();
				} else {
					toRemove.Add (define.Key);
				}
			}

			for ( int i = 0; i < toRemove.Count; i++ ) defines.Remove (toRemove[i]);
		}
		...
		public static void ApplyDefines (List<DefineObject> defines) {
			...
			for (int i=0;i<folders.Length;i++) {
				ApplyDefines (folders[i],defines);
			}
		}
